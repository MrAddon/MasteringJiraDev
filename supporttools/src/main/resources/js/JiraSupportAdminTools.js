// ==UserScript==
// @name         Jirasupport Tools for Jira Server
// @namespace    http://tampermonkey.net/
// @version      20.3
// @description  try to take over the world!
// @author       You
// @match        https://jira-test.edreamsodigeo.com/*
// @match        http://localhost:8080/*
// @match        http://testjirasolutions01.tecnofor.local:8081/*
// @grant        none
// ==/UserScript==
/* jshint -W097 */
'use strict';
...
function banner() {
    ...
    if (pathname == "/secure/admin/EditAnnouncementBanner!default.jspa" || pathname == "/jira/secure/admin/EditAnnouncementBanner!default.jspa") {

        var x = jQuery(".aui-navgroup-primary");
        var date = new Date().toLocaleString("en-US").replace(',','').trim();
        var input = jQuery("<input type='text' value='"+date+"' name='search_issuetype' autocomplete='on' autocorrect='off' autocapitalize='off' spellcheck='false' id='search_issuetype' class='search-entry text'></input>");
        var btn = jQuery("<button id='search_btn_issuetype' class='aui-button'>Set Countdown to Date</button>");
        var valor = "";
        var alerta = function(){
...
function customfields() {
    ...
    if (pathname == "/secure/admin/ViewCustomFields.jspa" || pathname == "/jira/secure/admin/ViewCustomFields.jspa") {
        ...
        var divs = document.getElementById('custom-fields-table').getElementsByTagName("tr");
        var options_custom = "";
        for (var i = 0; i < divs.length; i++) {
            var cf_id = divs[i].getAttribute("data-custom-field-id")
            ...
            try {
                if ( para[0] ) {
                    ...
                    options_custom = options_custom +"<option value='" + cf_id + "' />";
                    para[0].innerHTML = para[0].innerHTML + "(<b>ID:</b>" + cf_id + ")&nbsp;<a href='ConfigureCustomField!default.jspa?customFieldId="+cf_id +"' target='_new'>[CONFIG]</a> <a href='EditCustomField!default.jspa?id="+cf_id +"' target='_new'>[DESCRIPTION]</a> <a href='AssociateFieldToScreens!default.jspa?fieldId=customfield_"+cf_id +"' target='_new'>[SCREENS]</a>";
                }
            }catch (e) {
                if ( para[1] ) {
                    options_custom = options_custom +"<option value='" + para[1].getElementsByTagName("strong")[0].innerText + "' />";
                    options_custom = options_custom +"<option value='" + cf_id + "' />";
                    para[1].innerHTML = para[1].innerHTML + "(<b>ID:</b>" + cf_id + ")&nbsp;<a href='ConfigureCustomField!default.jspa?customFieldId="+cf_id +"' target='_new'>[CONFIG]</a> <a href='EditCustomField!default.jspa?id="+cf_id +"' target='_new'>[DESCRIPTION]</a> <a href='AssociateFieldToScreens!default.jspa?fieldId=customfield_"+cf_id +"' target='_new'>[SCREENS]</a>";
                }
            }
        }
...
function statuses() {
    ...
    if (pathname == "/secure/admin/ViewStatuses.jspa"|| pathname == "/jira/secure/admin/ViewStatuses.jspa") {
        var divs = document.getElementsByTagName('table')[0].getElementsByTagName("tr");
        var options_custom = "";
        for (var i = 0; i < divs.length; i++) {
            var para = divs[i].getElementsByTagName("td");
            if ( para[0] ) {
                ...
                para[0].innerHTML = para[0].innerHTML + "&nbsp;<a href='javascript:document.getElementById(\"search_status\").focus();document.getElementById(\"search_issuetype\").setSelectionRange(0, document.getElementById(\"search_issuetype\").value.length)'>[TOP]</a>";
...
function issuetypes() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/ViewIssueTypes.jspa"|| pathname == "/jira/secure/admin/ViewIssueTypes.jspa") {
...
function issuetypesscheme() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/ManageIssueTypeSchemes!default.jspa" || pathname == "/jira/secure/admin/ManageIssueTypeSchemes!default.jspa") {
...
function workflow() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/workflows/ListWorkflows.jspa" || pathname == "/jira/secure/admin/workflows/ListWorkflows.jspa") {
...
function views() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/ViewFieldScreens.jspa" || pathname == "/jira/secure/admin/ViewFieldScreens.jspa") {
...
function viewsscheme() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/ViewFieldScreenSchemes.jspa" || pathname == "/jira/secure/admin/ViewFieldScreenSchemes.jspa") {
...
function viewsschemeissue() {
    var pathname = window.location.pathname;
    if (pathname == "/secure/admin/ViewIssueTypeScreenSchemes.jspa" || pathname == "/jira/secure/admin/ViewIssueTypeScreenSchemes.jspa") {
...
function fields_settings() {
    ...
    if (pathname == "/secure/admin/ManageConfigurationScheme!default.jspa" || pathname == "/jira/secure/admin/ManageConfigurationScheme!default.jspa") {
        ...
        var btn = jQuery("<button id='save_issuetypes' class='aui-button'>Copy IssueTypes</button>");
        var btn2 = jQuery("<button id='load_issuetypes' class='aui-button'>Paste IssueTypes </button>");
        var btn3 = jQuery("<button id='save_projects' class='aui-button'>Copy Projects</button>");
        var btn4 = jQuery("<button id='load_projects' class='aui-button'>Paste Projects</button>");

        var alerta = function(){
            var valor = jQuery('select[name="issuetypes"]')[0];
            var selected1 = [];
        ...
        var alerta2 = function(){
            ...
            if (x) {
                var valores = x.split(",").reverse();
                for (var j = 0; j < valores.length; j++) {
                    var valor = null
                    if (JIRA.Version.isGreaterThanOrEqualTo("9.7.0")) {

                        document.getElementsByName('radio-issuetype')[1].click();
                        valor = jQuery('.issue-types-dual-list').find('ul')[1].children;

                        for (var i = 0; i < valor.length; i++) {
                            if (valor[i].getAttribute('data-rbd-draggable-id') == valores[j] ) {
                                valor[i].children[2].click();
                            }
                        }
                    } else {
        ...
        var alerta3 = function(){
            var valor = jQuery('select[name="projects"]')[0];
            var selected1 = [];
            for (var i = 0; i < valor.length; i++) {
                if (valor.options[i].selected) selected1.push(valor.options[i].value);
            }
            ...
            if (x) {
                ...
                for (var j = 0; j < valores.length; j++) {
                    ...
                    if (JIRA.Version.isGreaterThanOrEqualTo("9.7.0")) {
                        ...
                        document.getElementsByName('radio-project')[1].click();
                        valor = jQuery('.projects-dual-list').find('ul')[1].children;

                        for (var i = 0; i < valor.length; i++) {
                            if (valor[i].getAttribute('data-rbd-draggable-id') == valores[j] ) {
                                valor[i].children[3].click()
                            }
                        }
                    } else {
                        valor = document.getElementsByTagName('select')[1]
...
function customfields_call() {
    var x = jQuery("#add_custom_fields");
    var btn = jQuery("<button id='rauliki' class='aui-button'>JiraSupport [SUAJ]!</button>");
    var alerta = function(){
        //this.remove();
        customfields();
...
//Set the functions in each page
    ...
    } else if (pathname == "/secure/admin/ViewIssueTypes.jspa" || pathname == "/jira/secure/admin/ViewIssueTypes.jspa") {
        setTimeout(function() { issuetypes_call(); }, 60);
    } else if (pathname == "/secure/admin/ManageIssueTypeSchemes!default.jspa" || pathname == "/jira/secure/admin/ManageIssueTypeSchemes!default.jspa") {
    ...
    }  else if (pathname == "/secure/admin/workflows/ListWorkflows.jspa" || pathname == "/jira/secure/admin/workflows/ListWorkflows.jspa") {
        setTimeout(function() { workflow_call(); }, 60);
    } else if (pathname == "/secure/admin/ViewFieldScreens.jspa" || pathname == "/jira/secure/admin/ViewFieldScreens.jspa") {
        setTimeout(function() { views_call(); }, 60);
    } else if (pathname == "/secure/admin/ViewFieldScreenSchemes.jspa" || pathname == "/jira/secure/admin/ViewFieldScreenSchemes.jspa") {
        setTimeout(function() { viewsscheme_call(); }, 60);
    } else if (pathname == "/secure/admin/ViewIssueTypeScreenSchemes.jspa" || pathname == "/jira/secure/admin/ViewIssueTypeScreenSchemes.jspa") {
    ...
    } else if (pathname == "/secure/admin/EditAnnouncementBanner!default.jspa" || pathname == "/jira/secure/admin/EditAnnouncementBanner!default.jspa") {
        setTimeout(function() { banner_call(); }, 60);
...
function setbanner( valor ) {
    var javatxt = "\
\
<font color='red'><b>Today at "+valor+" we will RESTART JIRA due a maintenance</b></font>\
<script>\
var end = new Date('"+valor+"');\
\
var _second = 1000;\
...
\
clearInterval(timer);\
document.getElementById('countdown').innerHTML = '<b>RESTARTING JIRA!</b>';\
\
return;\
}\
...
\
document.getElementById('countdown').innerHTML = '<b>'+ days + ' days </b>';\
document.getElementById('countdown').innerHTML +='<b>'+  hours + ' hrs </b>';\
document.getElementById('countdown').innerHTML +='<b>'+  minutes + ' mins </b>';\
document.getElementById('countdown').innerHTML +='<b>'+  seconds + ' secs to RESTART</b>';\
}\
\
...
";
    document.getElementsByName('announcement')[0].value+= javatxt;
